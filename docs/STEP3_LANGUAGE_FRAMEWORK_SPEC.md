# Brewnet CLI — Step 3: Dev Stack & Runtime 언어/프레임워크 명세

> REQ-1.3.5, REQ-12.1 / USER-STORY §4
> **최종 수정**: 2026년 2월
> **기준**: 각 언어/프레임워크의 최신 안정 버전 (2026년 2월 기준)

---

## 개요

Step 3에서 사용자가 언어를 선택하면, 각 언어별로 **프레임워크 Sub-Prompt**가 즉시 표시된다.
...
| 필드 | 타입 | 기본값 | 옵션 |
|------|------|--------|------|
| `devStack.languages` | `string[]` | `[]` | `'python'` \| `'nodejs'` \| `'java'` \| `'kotlin'` \| `'php'` \| `'dotnet'` \| `'rust'` \| `'go'` |
...
| php | PHP 8.4 | 8.4.x | `php:8.4-fpm-alpine` | 8080 |
| dotnet | .NET 9 | 9.0.x | `mcr.microsoft.com/dotnet/aspnet:9.0` | 5000 |
...
> Sub-Prompt: `Select PHP framework:`

| Framework | 표시명 | 버전 | 라이선스 | 기본 포트 | 기본값 | 설명 |
|-----------|--------|------|---------|----------|:------:|------|
| `laravel` | Laravel | 12.x | MIT | 8080 | ✅ (recommended) | 가장 인기 있는 PHP 프레임워크. Eloquent ORM, Blade 템플릿, 큐, 스케줄러 내장 |
| `symfony` | Symfony | 8.0.x | MIT | 8080 | | 엔터프라이즈급 컴포넌트 기반 프레임워크. 재사용 가능한 PHP 라이브러리 집합 |

```
Select PHP framework:
  > Laravel  (default, recommended) — 풀스택, Eloquent ORM, 방대한 생태계
    Symfony  — 엔터프라이즈 컴포넌트 기반, 장기 유지보수
```
...
> Sub-Prompt: `Select .NET framework:`

| Framework | 표시명 | 버전 | 라이선스 | 기본 포트 | 기본값 | 설명 |
|-----------|--------|------|---------|----------|:------:|------|
| `aspnet-minimal` | ASP.NET Core Minimal API | 9.0.x | MIT | 5000 | ✅ (recommended) | 최소 보일러플레이트 API. Top-level statements, 간결한 엔드포인트 매핑 |
| `aspnet-mvc` | ASP.NET Core MVC | 9.0.x | MIT | 5000 | | 전통적 MVC 패턴. 컨트롤러/뷰/모델 구조, 대규모 웹 앱에 적합 |

```
Select .NET framework:
  > ASP.NET Minimal API  (default, recommended) — 최소 보일러플레이트, 간결한 API
    ASP.NET Core MVC     — 전통적 MVC 패턴, 대규모 웹 앱
```
...
const devStack: DevStack = {
  languages: ['python', 'nodejs', 'kotlin'],
  frameworks: {
    python: 'fastapi',
    nodejs: 'nextjs',
    kotlin: 'ktor'
...
Brewnet CLI는 각 프레임워크의 **최신 안정 버전(Latest Stable)**을 기본 템플릿으로 사용하며,
`brewnet update-templates` 명령어로 보일러플레이트 버전을 업데이트할 수 있다.

| 주기 | 내용 |
|------|------|
| 메이저 릴리즈 시 | 프레임워크 목록 및 기본값 검토 |
| 분기별 | Docker 베이스 이미지 버전 업데이트 |
| 수시 | 보안 패치 반영 |
