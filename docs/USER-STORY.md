...
## STEP 0: Installation
- 사용자는 brewnet의 github(https://github.com/claude-code-expert/brewnet.git) 에서 claude code 설치 방식과 동일한 curl -fsSL https://claude.ai/install.sh | bash 와 같은 형태로 brewnet을 터미널로 다운 받는다. 
- docker가 없을 경우 docker를 설치한 뒤 각 단계별 서버 프로그램들을 설치한다.
- 최종 설치 후 도메인까지 세팅이 완료되면 localhost 에 관리자 화면을 접속하여 서비스 되고 있는 프로그램들을 start, stop, install, uninstall 할 수 있다.  
