...
Before installing Brewnet, ensure the following are available on your machine:
...
|--------|-------------|----------------------|:------------:|
| **Full Install** | 모든 핵심 서버 컴포넌트를 포함한 완전한 설치 | Web + App + DB (PostgreSQL + Redis defaults) | Step 2 ~ 7 |
| **Partial Install** | Web Server만 필수, 나머지는 선택적으로 토글 | Web only (required) | Step 2 ~ 7 |

#### Scenario A: User selects "Full Install"

Full Install을 선택하면 Web Server, App Server, DB Server가 기본으로 활성화된다. DB Server는 PostgreSQL(Primary) + Redis(Cache)가 기본 선택된 상태로 시작된다. File Server와 Media는 비활성화 상태이며 사용자가 토글할 수 있다.

#### Scenario B: User selects "Partial Install"

Partial Install을 선택하면 Web Server만 필수(required)로 활성화된다. App Server, DB Server, File Server, Media 모두 비활성화 상태이며 사용자가 원하는 컴포넌트만 토글하여 구성할 수 있다.
...
이전 버전에서는 8개 카테고리에 약 40개 서비스를 선택하는 방식이었으나, 간소화된 구조에서는 **6개의 서버 컴포넌트 토글 카드**로 재구성되었다.
...
> 이 비밀번호는 Nextcloud admin, pgAdmin login, Jellyfin admin 등 모든 서비스의 기본 관리자 비밀번호로 전파된다. 서비스별로 개별 변경이 필요하면 설치 후 각 서비스의 관리 패널에서 변경할 수 있다.
...
├──────────────────────────────────────────────────────────────────┤
│  [ON]  App Server          (Optional)                             │
│        Toggle only — configured in Runtime step                   │
├──────────────────────────────────────────────────────────────────┤
...
> Full Install 선택 시 Web + App + DB가 기본 ON 상태이다. Partial Install 선택 시 Web만 ON이다.
...
### 3.5 App Server (Optional)

App Server 토글은 단순 on/off이다. 활성화하면 다음 단계(STEP 3: Runtime & Boilerplate)에서 언어/프레임워크를 선택하고 boilerplate를 구성한다.

```
? Enable App Server? (toggle)  [ON]

  App Server is enabled.
  Language and framework will be configured in the next step.
```

> Full Install에서는 기본 ON 상태이다.
...
> Admin credentials entered once → propagated to Nextcloud, pgAdmin, Jellyfin, SSH, Mail.

### 3.9 Component Summary
...
Selected Server Components:
  ...
  App Server:     Enabled (configure in next step)
  DB Server:      PostgreSQL + Redis
  Media:          Jellyfin
  SSH Server:     Enabled (port 2222, key-based auth, SFTP on)

  Credential Propagation:
    Admin credentials → Nextcloud, pgAdmin, Jellyfin, SSH, Mail

  Estimated Resources:
    Containers: 6 (+ app container in next step)
    RAM:        ~3.5GB
...
## STEP 4: Runtime & Boilerplate (STEP 3/7)

```
┌─────────────────────────────────────────────────────────────────┐
│  STEP 3/7: Runtime & Boilerplate                                 │
└─────────────────────────────────────────────────────────────────┘

Select the language and framework for your API server.
A production-ready boilerplate will be generated.
```

> 이 단계는 App Server가 활성화된 경우에만 표시된다. App Server가 비활성화 상태이면 STEP 4로 건너뛴다.
...
**Example selection**: User selects `Python`.
...
? Select Node.js framework:
    ...
    Next.js            — React full-stack framework (MIT)
...
? Select Java framework:
    ...
    Spring Boot        — Enterprise Java framework (Apache-2.0)
    Quarkus            — Cloud-native Java (Apache-2.0)
    Micronaut          — Lightweight framework (Apache-2.0)
```
...
Starting services...
  ...
  Running health checks...
    [OK] All 9 containers are healthy
...
╚══════════════════════════════════════════════════════════════════╝
  ...
  Used by:          Nextcloud, pgAdmin, Jellyfin, etc.
...
### 8.1 Local DNS Setup (Local Only 선택 시)
...
Full Install은 가장 일반적인 설치 방식이다. Web + App + DB가 기본으로 활성화되며, DB는 PostgreSQL + Redis가 기본 선택된다. 사용자는 File Server(Nextcloud/MinIO)와 Media(Jellyfin)를 추가로 토글할 수 있다.

```
Default ON:   Web Server (Traefik) + App Server + DB Server (PostgreSQL + Redis)
Default OFF:  File Server, Media
```
...
Partial Install은 최소한의 구성으로 시작하고 싶은 사용자를 위한 방식이다. Web Server만 필수이며, 나머지 모든 컴포넌트를 선택적으로 토글할 수 있다.

```
Required:     Web Server (Traefik/Nginx/Caddy)
...
| **DB Server -- Primary** | PostgreSQL, MySQL, MariaDB, SQLite | PostgreSQL (recommended) |
...
| STEP 3 (2/7) | Server Components | Admin 계정 설정 + 6개 서버 컴포넌트 토글 카드 (Web/File/App/DB/Media/SSH), credential propagation |
| STEP 4 (3/7) | Runtime & Boilerplate | 언어/프레임워크 선택, boilerplate 옵션 (App Server 활성화 시) |
