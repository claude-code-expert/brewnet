...
|--------|-------------|----------------------|:------------:|
| **Full Install** | 모든 핵심 서버 컴포넌트를 포함한 완전한 설치 | Web + Git + App + DB (PostgreSQL + Redis defaults) | Step 2 ~ 7 |
| **Partial Install** | Web + Git Server 필수, 나머지는 선택적으로 토글 | Web + Git (required) | Step 2 ~ 7 |

#### Scenario A: User selects "Full Install"

Full Install을 선택하면 Web Server, Git Server, App Server, DB Server가 기본으로 활성화된다. Web Server와 Git Server는 항상 필수(required)이다. DB Server는 PostgreSQL(Primary) + Redis(Cache)가 기본 선택된 상태로 시작된다. App Server 선택 시 FileBrowser가 자동 포함된다. File Server와 Media는 비활성화 상태이며 사용자가 토글할 수 있다.
...
### 4.4 App Storage — FileBrowser (기본 포함)

App Server 활성화 시 FileBrowser(`filebrowser/filebrowser:latest`)가 기본으로 포함된다. 웹 기반 파일 매니저로 파일 업로드/다운로드/관리 기능을 제공하며, Admin credentials로 로그인하여 사용한다. 사용자별 `scope` 설정으로 디렉토리 격리가 가능하다.
...
Full Install은 가장 일반적인 설치 방식이다. Web + Git + App + DB가 기본으로 활성화되며, DB는 PostgreSQL + Redis가 기본 선택된다. 사용자는 File Server(Nextcloud/MinIO)와 Media(Jellyfin)를 추가로 토글할 수 있다. App Server 선택 시 FileBrowser가 자동으로 포함된다.

```
Required:     Web Server (Traefik) + Git Server (Gitea)
Default ON:   App Server + DB Server (PostgreSQL + Redis)
...
| STEP 3 (2/7) | Server Components | Admin 계정 설정 + 7개 서버 컴포넌트 토글 카드 (Web/File/App/Git/DB/Media/SSH), credential propagation |
