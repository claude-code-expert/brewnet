...
**9단계** 대화형 위저드가 시작됩니다 (Pre-Step + Step 0~7).

### 위저드 단계별 안내

#### Pre-Step — 관리자 계정 설정 (NEW v2.3)
> Docker 설치 **이전**에 admin 계정을 먼저 설정합니다.
...
> **포트 충돌 시 (NEW v2.3)**: 사용 중인 포트가 감지되면 자동으로 대안 포트를 제안합니다.
> (예: 포트 80 사용 중 → 8080, 8088, 8000 중 선택)
...
- Docker 이미지 pull
- 서비스 시작 (의존성 순서 자동 처리)
- **서비스별 접속 확인 (NEW v2.3)**: 각 서비스 시작 후 접속 URL 표시 및 헬스체크

#### Step 7 — 완료
- **로컬 접속 URL + 외부 접속 URL 모두 표시 (NEW v2.3)**
- 관리자 계정 정보 표시
...
3. 프로젝트 디렉터리 (`~/brewnet/<프로젝트명>`)
4. `~/.brewnet/status`, `~/.brewnet/state`
5. 위저드 상태 파일 (`~/.brewnet/projects/<name>/`)
6. **[NEW v2.3] Docker 제거 여부 확인 프롬프트**:

```
  All services have been removed.

  ? Remove Docker installation? (y/N)
    macOS: brew uninstall --cask docker
    Linux: sudo apt-get purge docker-ce docker-ce-cli

  → y 선택 시 Docker 자동 제거
  → N 선택 시 Docker 유지 (기본값)
```
...
| 명령어 | 설명 |
|--------|------|
| `brewnet uninstall` | 전체 제거 (데이터 포함) + Docker 제거 여부 확인 |
...
| `brewnet uninstall --remove-docker` | **[NEW v2.3]** 서비스 제거 후 Docker도 자동 제거 |
...
# 서비스 + Docker까지 완전 정리 (재설치 불필요 시)
brewnet uninstall --remove-docker
```
...
**Q: 왜 Docker 설치 전에 admin 계정을 먼저 설정하나요?**
A: Docker 설치가 완료되면 즉시 컨테이너를 기동해야 합니다. 기동 시 admin 크리덴셜이 모든 서비스 환경 변수에 미리 설정되어 있어야 원활한 초기화가 가능합니다.

**Q: 포트 80이 이미 사용 중인데 어떻게 하나요?**
A: `brewnet init` Step 0에서 포트 충돌을 감지하고 자동으로 대안 포트(8080, 8088, 8000 등)를 제안합니다. 원하는 포트를 선택하면 `docker-compose.yml`에 자동 적용됩니다.
...
*Brewnet · MIT License · [GitHub](https://github.com/claude-code-expert/brewnet)*
