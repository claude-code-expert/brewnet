...
Brewnet은 셀프 호스팅 홈 서버 관리 플랫폼입니다. 단일 CLI 명령어와 웹 기반 대시보드를 통해 Docker 기반 서비스들을 손쉽게 설치, 관리, 배포할 수 있습니다. Git 서버, 파일 저장소, 데이터베이스, 리버스 프록시, 미디어 서버, 그리고 Cloudflare Tunnel을 통한 외부 접근까지 하나의 인터랙티브 마법사로 모두 구성할 수 있습니다.
