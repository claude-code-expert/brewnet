...
/**
 * Allowed TLDs for the DigitalPlat free-domain service.
 */
const ALLOWED_FREE_DOMAIN_TLDS = ['.dpdns.org', '.qzz.io', '.us.kg'] as const;

export type FreeDomainTld = (typeof ALLOWED_FREE_DOMAIN_TLDS)[number];

/**
 * Validate that a TLD is one of the allowed free-domain options.
 *
 * Accepted values:
 *   - `.dpdns.org`
 *   - `.qzz.io`
 *   - `.us.kg`
 */
export function validateFreeDomainTld(tld: string): ValidationResult {
  if (typeof tld !== 'string' || tld.length === 0) {
    return { valid: false, error: 'Free domain TLD is required' };
  }
  ...
  if (
    !ALLOWED_FREE_DOMAIN_TLDS.includes(normalized as FreeDomainTld)
  ) {
    return {
      valid: false,
      error: `Invalid free domain TLD "${tld}". Allowed values: ${ALLOWED_FREE_DOMAIN_TLDS.join(', ')}`,
    };
  }
