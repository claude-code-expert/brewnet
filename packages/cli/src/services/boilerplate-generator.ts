...
function nextjsApiTemplate(): ScaffoldTemplate {
  return {
    appDir: 'nextjs-api',
    files: [
      {
...
function fastifyTemplate(): ScaffoldTemplate {
  return {
    appDir: 'fastify-app',
    files: [
      {
...
function laravelTemplate(): ScaffoldTemplate {
  return {
    appDir: 'laravel-app',
    files: [
      {
        ...
        template: JSON.stringify(
          {
            name: 'brewnet/${PROJECT_NAME}',
            type: 'project',
            description: '${PROJECT_NAME} - Powered by Brewnet',
            require: {
              php: '^8.3',
      ...
      {
        path: '.dockerignore',
        template: ['vendor', '.git', 'Dockerfile', '.dockerignore', 'node_modules', ''].join('\n'),
      },
    ],
  };
}

function symfonyTemplate(): ScaffoldTemplate {
  return {
    appDir: 'symfony-app',
    files: [
      {
      ...
      {
        path: '.dockerignore',
        template: ['vendor', '.git', 'Dockerfile', '.dockerignore', 'var', ''].join('\n'),
      },
    ],
  };
}
...
function aspnetTemplate(): ScaffoldTemplate {
  return {
    appDir: 'aspnet-app',
    files: [
      {
        path: '${PROJECT_NAME}.csproj',
        template: [
      ...
      {
        path: '.dockerignore',
        template: ['bin', 'obj', '.git', 'Dockerfile', '.dockerignore', ''].join('\n'),
      },
    ],
  };
}

function blazorTemplate(): ScaffoldTemplate {
  return {
    appDir: 'blazor-app',
    files: [
      {
...
function getSampleDataFiles(
  ...
  switch (language) {
          ...
          return [
            {
              ...
              template: [
                ...
                'const SAMPLE_USERS = [',
                '  { id: 1, name: "${ADMIN_USER}", role: "admin" },',
                '  { id: 2, name: "user1", role: "member" },',
                '  { id: 3, name: "user2", role: "member" },',
                '];',
                '',
                'export async function GET() {',
    ...
    case 'php':
      return [
        {
          path: 'database/seeders/demo.json',
          template: JSON.stringify(
            {
              users: [
                { name: '${ADMIN_USER}', role: 'admin' },
                { name: 'demo', role: 'viewer' },
              ],
            },
            null,
            2,
...
function getDevConfig(language: Language, framework: string): DevConfig {
  switch (language) {
    ...
    case 'php':
      switch (framework) {
        case 'laravel':
          return { srcMount: '.', containerMount: '/app', command: 'php artisan serve --host=0.0.0.0 --port=8000' };
        case 'symfony':
          return { srcMount: 'src', containerMount: '/app/src', command: 'php -S 0.0.0.0:8000 -t public' };
        default:
          return { srcMount: '.', containerMount: '/app', command: 'php -S 0.0.0.0:8000' };
      }

    case 'dotnet':
      return { srcMount: '.', containerMount: '/app', command: 'dotnet watch run --urls http://0.0.0.0:5000' };
...
export function getScaffoldTemplate(
  ...
  switch (language) {
    ...
    case 'dotnet':
      switch (framework) {
        case 'aspnet':
          return aspnetTemplate();
        case 'blazor':
          return blazorTemplate();
        default:
          return aspnetTemplate();
      }
