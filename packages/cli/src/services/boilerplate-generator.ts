...
// ---------------------------------------------------------------------------
// Kotlin Templates (Ktor + Spring Boot Kotlin)
// ---------------------------------------------------------------------------

function ktorTemplate(): ScaffoldTemplate {
  return {
    appDir: 'ktor-app',
    files: [
      {
      ...
      },
      {
        ...
        template: 'rootProject.name = "${PROJECT_NAME}-ktor-backend"\n',
      },
      {
        path: 'src/main/kotlin/dev/brewnet/app/Application.kt',
        template: [
      ...
      {
        path: 'src/main/kotlin/dev/brewnet/app/plugins/Routing.kt',
        template: [
      ...
      {
        path: 'src/main/kotlin/dev/brewnet/app/plugins/Database.kt',
        template: [
          'package dev.brewnet.app.plugins',
          '',
          'import io.ktor.server.application.*',
          ...
          '        org.jetbrains.exposed.sql.transactions.transaction { exec("SELECT 1") }',
          '        true',
          '    } catch (e: Exception) {',
          '        false',
          '    }',
      ...
      {
        path: 'src/main/kotlin/dev/brewnet/app/plugins/Serialization.kt',
        template: [
          'package dev.brewnet.app.plugins',
          '',
      ...
      },
      {
        ...
        template: [
          ...
          '',
          'FROM eclipse-temurin:21-jre-alpine',
          'RUN apk --no-cache add wget && adduser -D -h /app appuser',
          'WORKDIR /app',
      ...
      {
        path: '.dockerignore',
        template: ['build', '.gradle', '.git', '*.md', '.env*', '.vscode', '.idea', ''].join('\n'),
      },
    ],
  };
}

function springbootKtTemplate(): ScaffoldTemplate {
  return {
    appDir: 'springboot-kt-app',
    files: [
      {
        path: 'build.gradle.kts',
        template: [
      ...
      {
        path: 'settings.gradle.kts',
        template: 'rootProject.name = "${PROJECT_NAME}-springboot-kt-backend"\n',
      },
      {
        path: 'src/main/kotlin/dev/brewnet/app/Application.kt',
        template: [
      ...
      {
        path: 'src/main/kotlin/dev/brewnet/app/controller/ApiController.kt',
        template: [
      ...
      {
        path: 'src/main/kotlin/dev/brewnet/app/config/DataSourceConfig.kt',
        template: [
...
function getDevConfig(language: Language, framework: string): DevConfig {
  switch (language) {
    ...
    case 'kotlin':
      switch (framework) {
        case 'springboot-kt':
          return { srcMount: 'src', containerMount: '/app/src', command: 'gradle bootRun' };
        case 'ktor':
        default:
          return { srcMount: 'src', containerMount: '/app/src', command: 'gradle run' };
      }
